...
t_std_error sdi_entity_reset(sdi_entity_hdl_t hdl, sdi_reset_type_t type)
{
    ...
    if (sdi_is_entity_feature_support(entity_priv_hdl, type) == true) {
        ...
        if (entity_priv_hdl->reset_pin_grp_hdl_l1 != NULL) {
            ret = sdi_pin_group_acquire_bus(entity_priv_hdl->reset_pin_grp_hdl_l1);
            if (ret != STD_ERR_OK) {
                return ret;
            }

            ret = sdi_pin_group_write_level(entity_priv_hdl->reset_pin_grp_hdl_l1,
                                            entity_priv_hdl->reset_value_l1);

            sdi_pin_group_release_bus(entity_priv_hdl->reset_pin_grp_hdl_l1);
