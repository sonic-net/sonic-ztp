...
sdi_entity_hdl_t sdi_entity_create(sdi_entity_type_t type, uint_t instance, const char *name)
{
    ...
    entity_hdl->reset_pin_grp_hdl_l1 = NULL;
...
static void sdi_fill_reset_info(std_config_node_t node, sdi_entity_hdl_t entity_hdl)
{
    ...
    config_attr = std_config_attr_get(node, "reset_register_l1");
    if (config_attr != NULL) {
        entity_priv_hdl->reset_pin_grp_hdl_l1 =
            sdi_get_pin_group_bus_handle_by_name(config_attr);
    }

    config_attr = std_config_attr_get(node, "reset_value_l1");
    if (config_attr != NULL) {
        entity_priv_hdl->reset_value_l1 = strtoul(config_attr, NULL, 0x10);
    }
